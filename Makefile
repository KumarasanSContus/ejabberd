...
ESCRIPT = /usr/local/lib/erlang/bin/escript
REBAR = $(MAYBE) mix # rebar|rebar3|mix binary (or path to binary)
REBAR3 = /usr/local/bin/rebar3 # path to rebar3 binary
MIX = mix
AWK = awk
...
# /lib/ejabberd/
EJABBERDDIR = ${exec_prefix}/lib/ejabberd
...
# /var/lib/ejabberd/
SPOOLDIR = ${prefix}/var/lib/ejabberd

# /var/log/ejabberd/
LOGDIR = ${prefix}/var/log/ejabberd
...
ifeq "$(REBAR_VER_318)" "1"
  ...
  GET_DEPS= get-deps
  CONFIGURE_DEPS=sh -c 'for d in $$(find _build/default/lib -maxdepth 1 -type d); do if [ -f $$$$d/configure ]; then (cd $$$$d && ./configure); fi; done'
  EBINDIR=$(DEPSDIR)/ejabberd/ebin
  XREFOPTIONS=
  CLEANARG=--all
  ...
  DEPSDIR=$(DEPSBASE)
  GET_DEPS= get-deps
  CONFIGURE_DEPS=sh -c 'for d in $$(find deps -maxdepth 1 -type d); do if [ -f $$$$d/configure ]; then (cd $$$$d && ./configure); fi; done'
  EBINDIR=ebin
  XREFOPTIONS=
  CLEANARG=
